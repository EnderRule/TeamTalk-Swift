...
- (void)calculatePageNumbers
{
    ...
    //注意，這裡設置delegate是因為計算contentsize的時候會引起scrollViewDidScroll執行，修改currentpage的值，這樣在貼圖（舉個例子）前面的分類頁數比後面的分類頁數多，前面的分類滾動到最後面一頁後，再顯示後面的分類，會顯示不正確
