...
//    self.encapsulator.delegete = self;
    ...
    self.timerTimeout = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(timeoutCheck:) userInfo:nil repeats:YES];
...
- (void)encapsulatingOver {
    if (self.delegate && [self.delegate respondsToSelector:@selector(recordingFinishedWithFileName:time:)]) {
...
- (void)timeoutCheck:(id)sender {
    if (dateStartRecording){
        NSTimeInterval time =  [[NSDate date] timeIntervalSinceDate:dateStartRecording];
        
        if (self.delegate){
            if ([self.delegate respondsToSelector:@selector(recordingProgress:)]){
                [self.delegate recordingProgress:time];
            }
            
            if (time > maxRecordTime && [self.delegate respondsToSelector:@selector(recordingTimeout)]){
