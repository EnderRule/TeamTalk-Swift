...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        self.maxImagesCount = maxImagesCount > 0 ? maxImagesCount : 9; // Default is 9 / 默認最大可選9張圖片
        self.pickerDelegate = delegate;
        self.selectedModels = [NSMutableArray array];
        
        // Allow user picking original photo and video, you also can set No after this method
        // 默認準許用戶選擇原圖和視頻, 你也可以在這個方法後置為NO
...
- (void)pushPhotoPickerVc {
    ...
    // 1.6.8 判斷是否需要push到照片選擇頁，如果_pushPhotoPickerVc為NO,則不push
...
- (void)setAllowCrop:(BOOL)allowCrop {
    ...
    if (allowCrop) { // 允許裁剪的時候，不能選原圖和GIF
...
- (void)viewDidLoad {
    ...
    // 1.6.10 採用微信的方式，只在相冊列表頁定義backBarButtonItem為返回，其餘的順系統的做法
