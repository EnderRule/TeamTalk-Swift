...
+ (instancetype)manager {
    ...
    dispatch_once(&onceToken, ^{
        ...
        // 測試發現，如果scale在plus真機上取到3.0，內存會增大特別多。故這裡寫死成2.0
        TZScreenScale = 2.0;
    ...
    if (iOS8Later) {
        ...
        for (PHFetchResult *fetchResult in allAlbums) {
            for (PHAssetCollection *collection in fetchResult) {
                ...
                if ([collection.localizedTitle containsString:@"Deleted"] || [collection.localizedTitle isEqualToString:@"最近刪除"]) continue;
                if ([self isCameraRollAlbum:collection.localizedTitle]) {
...
///  Get asset at index 獲得下標為index的單個照片
///  if index beyond bounds, return nil in callback 如果索引越界, 在回調中返回 nil
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        // 修復獲取圖片時出現的瞬間內存過高問題
        // 下面兩行代碼，來自hsjcom，他的github是：https://github.com/hsjcom 表示感謝
...
- (void)savePhotoWithImage:(UIImage *)image completion:(void (^)(NSError *error))completion {
    ...
    if ([PHPhotoLibrary authorizationStatus] != PHAuthorizationStatusAuthorized  ) {
        NSError *error = [NSError errorWithDomain:@"systom.authorization" code:33033 userInfo:@{NSLocalizedDescriptionKey:@"手機相冊訪問受限",NSLocalizedFailureReasonErrorKey:@"手機相冊訪問受限"}];
        completion(error);
    }else{
        if (iOS9Later) { // 這裡有坑... iOS8系統下這個方法保存圖片會失敗 原來是因為PHAssetResourceType是iOS9之後的...
...
- (BOOL)isCameraRollAlbum:(NSString *)albumName {
    ...
    // 目前已知8.0.0 - 8.0.2系統，拍照後的圖片會保存在最近添加中
    if (version >= 800 && version <= 802) {
        return [albumName isEqualToString:@"最近添加"] || [albumName isEqualToString:@"Recently Added"];
    } else {
        return [albumName isEqualToString:@"Camera Roll"] || [albumName isEqualToString:@"相機膠卷"] || [albumName isEqualToString:@"所有照片"] || [albumName isEqualToString:@"All Photos"];
    }
}
