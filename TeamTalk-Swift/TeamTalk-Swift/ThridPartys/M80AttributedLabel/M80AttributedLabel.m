...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        //如果LineBreakMode為TranncateTail,那麼默認排版模式改成kCTLineBreakByCharWrapping,使得盡可能地顯示所有文字
        ...
        CGFloat fontLineHeight = self.font.lineHeight;  //使用全局fontHeight作為最小lineHeight
...
- (M80AttributedLabelURL *)urlForPoint:(CGPoint)point
{
    ...
    CGFloat verticalOffset = 0; //不像Nimbus一樣設置文字的對齊方式，都統一是TOP,那麼offset就為0
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    if (framesetter) {
        ...
        //hack:
        //1.需要加上額外的一部分size,有些情況下計算出來的像素點並不是那麼精準
        //2.iOS7 的 CTFramesetterSuggestFrameSizeWithConstraint s方法比較殘,需要多加一部分 height
        //3.iOS7 多行中如果首行帶有很多空格，會導致返回的 suggestionWidth 遠小於真實 width ,那麼多行情況下就是用傳入的 width
        if (newSize.height < _fontHeight * 2)   //單行
        {
        ...
        return CGSizeMake(size.width, ceilf(size.height) + 4.0);
    }
}
            ...
            for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++)
            {
                    ...
                    _lineBreakMode == kCTLineBreakByTruncatingTail)
                {
                    ...
                    if (lastLineRange.location + lastLineRange.length < attributedString.length)
                    {
                        ...
                        {
                            //移除掉最後一個對像...（其實這個地方有點問題,也有可能需要移除最後 2 個對象，因為 attachment 寬度的關係）
