...
#else
    int EncryptMsg(const char* pInData, int nInLen, char** ppOutData, uint32_t** nOutLen)
    {
        if(pInData == NULL|| nInLen <=0 )
        {
            return -1;
        ...
        const char *key = "6f1cd98c5655da86d60a73effae355eb";//"12345678901234567890123456789012";
        AES_set_encrypt_key((const unsigned char*)key, 256, &aesKey);
        for (uint32_t i = 0; i < nBlocks; i++) {
            AES_encrypt(pData + i * 16, pEncData + i * 16, &aesKey);
        }
...
//        *nOutLen = (uint32_t *)strDec.length();
        ...
        pTmp[strDec.length()] = 0;
        *ppOutData = pTmp;
        return 0;
    }
    
    int DecryptMsg(const char* pInData, int nInLen, char** ppOutData,uint32_t** nOutLen)
    {
        if(pInData == NULL|| nInLen <=0 )
        {
            return -1;
        ...
        uchar_t* pStart = (uchar_t*)pTmp+nLen-4;
        uint32_t theOutLen = (uint32_t) ReadUint32(pStart);
    ...
    int EncryptPass(const char* pInData, int nInLen, char** ppOutData, uint32_t** nOutLen)
    {
        if(pInData == NULL|| nInLen <=0 )
        {
            return -1;
