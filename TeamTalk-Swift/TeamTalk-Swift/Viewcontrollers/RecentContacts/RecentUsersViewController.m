...
-(void)getMacLoginStatus{
    ...
    UInt32 uid = [MTTUserEntity pbIDFromLocalID:TheRuntime.user.objID];// [MTTUserEntity localIDTopb:TheRuntime.user.objID];
    [array addObject:@(uid)];
    [request requestWithObject:array Completion:^(NSArray *response, NSError *error) {
        if(response){
            ...
            if([narray[0][1] intValue] == DDUserOnline){// UserStatTypeUserStatusOnline){
                self.isMacOnline = 1;
            }
...
{
    if([[[notification object]objectForKey: @"loginStat"] intValue]== DDUserOffLine){
        self.isMacOnline = 0;
    }else{
        self.isMacOnline = 1;
    }
