...
- (void)viewDidLoad
{
    ...
    // 普通可以加,自建可以编辑
    // 个人只可以加.
    if(self.session.isGroupSession){
        if (self.group.groupType == GroupType_ObjcGroupTypeNormal) {
...
-(void)clickDeleteUser:(id)sender
{
    ...
    DeleteMemberFromGroupAPI* deleteMemberAPI = [[DeleteMemberFromGroupAPI alloc] init];
