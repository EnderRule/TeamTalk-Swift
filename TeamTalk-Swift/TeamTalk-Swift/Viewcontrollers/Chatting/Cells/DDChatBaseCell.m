...
- (void)setContent:(MTTMessageEntity*)content
{
    ...
    //设置昵称是否隐藏
    if (self.session.sessionType == SessionType_ObjcSessionTypeSingle || self.location == DDBubbleRight) {
    ...
    switch (content.msgContentType) {
        case DDMessageContentTypeImage:
            showMenu = showMenu | DDShowPreview;
            break;
        case DDMessageContentTypeText:
            showMenu = showMenu | DDShowCopy;
            break;
        case DDMessageContentTypeVoice:
...
{
    _leftConfig =  [[[MTTBubbleModule alloc]init] getBubbleConfigLeft:YES];//  [[MTTBubbleModule shareInstance] getBubbleConfigLeft:YES];
    _rightConfig = [[[MTTBubbleModule alloc]init] getBubbleConfigLeft:NO];//[[MTTBubbleModule shareInstance] getBubbleConfigLeft:NO];
    return 0;
}
