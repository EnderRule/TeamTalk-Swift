...
- (void)setContent:(MTTMessageEntity*)content
{
    ...
    if(content.msgContentType == DDMessageContentTypeImage)
    {
        ...
        if (messageContent[ MTTMessageEntity.DD_IMAGE_LOCAL_KEY])
        {
            //加载本地图片
            NSString* localPath = messageContent[MTTMessageEntity.DD_IMAGE_LOCAL_KEY];
            ...
            //加载服务器上的图片
            NSString* url = messageContent[MTTMessageEntity.DD_IMAGE_URL_KEY];
...
- (void)sendImageAgain:(MTTMessageEntity*)message
{
    ...
    NSString* locaPath = dic[MTTMessageEntity.DD_IMAGE_LOCAL_KEY];
