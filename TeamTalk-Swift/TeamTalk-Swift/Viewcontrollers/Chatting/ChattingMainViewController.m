...
{
    static ChattingMainViewController *shared;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared = [[ChattingMainViewController alloc]init];
    });
    return shared;
}
...
{
    NSDictionary* messageContentDic = @{MTTMessageEntity.DD_IMAGE_LOCAL_KEY:photo.localPath};
    NSString* messageContent = [messageContentDic jsonString];
    
     MTTMessageEntity *message =  [[MTTMessageEntity alloc]initWithContent:messageContent module:self.module msgContentType:DDMessageContentTypeImage];
    ...
    [[DDSendPhotoMessageAPI sharedPhotoCache] uploadImage:messageContentDic[MTTMessageEntity.DD_IMAGE_LOCAL_KEY] success:^(NSString *imageURL) {
        [self scrollToBottomAnimated:YES];
        message.state= DDMessageStateSending;
        ...
        [mutalMessageContent setValue:imageURL forKey:MTTMessageEntity.DD_IMAGE_URL_KEY];
...
- (void)textViewChanged
{
    ...
    DDLog(@"textView Enter changed:%@",text);
...
- (void)textViewEnterSend
{
    ...
    DDLog(@"textView Enter Send:%@",text);
    ...
    DDMessageContentType msgContentType = DDMessageContentTypeText;
    MTTMessageEntity *message =[[MTTMessageEntity alloc]initWithContent:text module:self.module msgContentType:msgContentType];// [MTTMessageEntity makeMessage:text Module:self.module MsgType:msgContentType];
    [[MTTDatabaseUtil instance] insertMessages:@[message] success:^{
        DDLog(@"消息插入DB成功");
    } failure:^(NSString *errorDescripe) {
        DDLog(@"消息插入DB失败");
...
{
    BOOL isGroup = [self.module.SessionEntity isGroupSession];
    [[DDMessageSendManager instance] sendMessage:message isGroup:isGroup Session:self.module.SessionEntity  completion:^(MTTMessageEntity* theMessage,NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
//    self.tableView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 64 * 2);
...
-(void)queryUserStat
{
    UserStatAPI *request = [UserStatAPI new];
    NSMutableArray *array = [NSMutableArray new];
    NSString *sessionId = self.module.SessionEntity.sessionID;
    UInt32 uid = [MTTUtil changeIDToOriginal:sessionId];// localIDTopb:sessionId];
    [array addObject:@(uid)];
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    {
        NSString *sessionId = self.module.SessionEntity.sessionID;
        self.ddUtility.userId = [MTTUtil changeIDToOriginal:sessionId];//// localIDTopb:sessionId];
    }
}
...
//    [self.tableView scrollRectToVisible:CGRectMake(0, self.tableView.contentSize.height - self.tableView.frame.size.height - 2, self.tableView.size.width, 1) animated:animated];
